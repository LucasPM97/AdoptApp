...
class MockPetsDataSource : IPetsDataSource {
    ...
    private fun mockedPetList(): List<PetItem> {
        ...
        for (i in (1..10)) {
            mockedList.add(
                PetItem(
                    ...
                    birthDate = LocalDate.parse(getRandomDate()),
                    publishDate = LocalDate.parse(getRandomDate()),
                    location = "$i km"
                )
            )
        }
    ...
    private fun getRandomDate(): String =
        "${yearList.random()}-${monthList.random()}-${dayList.random()}"
